...
def test(
    ...
    print(
        f"Testing Sin on {InfiniDeviceNames[device]} with shape:{shape} x_stride:{x_stride} y_stride:{y_stride} "
        f"dtype:{InfiniDtypeNames[dtype]} inplace:{inplace}"
    )

    sin(y.torch_tensor(), x.torch_tensor())
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: sin(y.torch_tensor(), x.torch_tensor()), device, NUM_PRERUN, NUM_ITERATIONS)
