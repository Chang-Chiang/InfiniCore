...
struct Test::Attributes {
    ...
    std::shared_ptr<Tensor> neagative_slope;
...
std::shared_ptr<Test> Test::build(
    ...
    if (tensors.find("x") == tensors.end()
        || tensors.find("neagative_slope") == tensors.end()
        || tensors.find("y") == tensors.end()) {
        throw std::runtime_error("Invalid Test");
    }

    test->_attributes->x = tensors["x"];
    test->_attributes->neagative_slope = tensors["neagative_slope"];
